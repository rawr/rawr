module Rawr
  class Command
    ...
    def self.compile_ruby_dirs(src_dirs, dest_dir, jruby_jar='lib/java/jruby-complete.jar', exclude=[], target_jvm='1.5', copy_only=false)
      ...
      sh "java -jar #{jruby_jar} -e \"require '#{File.expand_path(File.dirname(__FILE__))}/jruby_batch_compiler'; Rawr::JRubyBatchCompiler.compile_argv\" #{src_dirs.join(' ')} #{dest_dir}"
    end
